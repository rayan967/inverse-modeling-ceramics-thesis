...
""" 
'JCM imports'
from jcm.model_standin.model_standin.gpytorch_multichannel_6D import *
import torch


def check_parameter_is_valid(h, cd, swa, t, r_top, r_bot, pitch=50):
    """Checks whether or not the parameter in question is geometrically valid or
    not.

    Based on constraints in the original MATLAB code:

    % t - r_top < 0
    constraints(1).name = 'r_top_t';
    constraints(1).constraint = 't - r_top + 1';

    % r_top < cd/2 -h/2/tand(swa) <=> cd/2 -h/2/tand(swa) -r  > 0
    constraints(2).name = 'r_top';
    constraints(2).constraint = 'r_top - cd/2 + h/2/tan(Pi*swa/180) + 1';

    % r_bottom < pitch/2 - cd/2 -h/2/tand(swa) <=> (pitch -cd -h/tand(swa))/2 -r_bottom  > 0
    constraints(3).name = 'r_bottom';
    constraints(3).constraint = ['r_bot - pitch/2 + cd/2 + h/2/tan(Pi*swa/180) + 1'];

    % r_bottom +t < pitch/2 - cd/2 -h/2/tand(swa)+t*tand(swa/2) <=> pitch/2 -cd/2 -h/2/tand(swa)+(tand(swa/2)-1)*t -r_bottom  > 0
    constraints(4).name = 'r_bottom_t';
    constraints(4).constraint = ['r_bot + t - pitch/2 + cd/2 + ' ...
                    'h/2/tan(Pi*swa/180) - t*tan(Pi*swa/2/180) + 1'];

    """
    # adding 1 to each constraint makes them a bit stronger, such that fewer
    # samples are accepted as valid
    constraint_1 = t - r_top  # + 1
    constraint_2 = r_top - cd / 2 + h / 2 / np.tan(np.deg2rad(swa))  # + 1
    constraint_3 = r_bot - pitch / 2 + cd / 2 + h / 2 / np.tan(np.deg2rad(swa))  # + 1
    constraint_4 = (
        r_bot
        + t
        - pitch / 2
        + cd / 2
        + h / 2 / np.tan(np.deg2rad(swa))
        - t * np.tan(np.deg2rad(swa / 2))
    )  # + 1

    constraint_sum = np.sum(
        np.sign([constraint_1, constraint_2, constraint_3, constraint_4])
    )
...
def adapt(gp, totalbudget, incrementalbudget, parameterranges,
    ...
    "Create JCM problem"
    coarse_data_csv = "F:/Uni/Zuse/simlopt/simlopt/jcm/model_standin/model_standin/results_fine_6D.csv"
    coarse_gp = MultChGP(coarse_data_csv) 
    ...
    while currentcost < totalbudget:
        ...
        if sol.success == True:
            ...
            if indicesofchangedpoints[1].size == 0:
                ...
                for jj in range(indicesofchangedpoints[1].shape[0]):
                    ...
                    print("Solve simulation for swa: {} and h: {}".format(swa,h)) 
                    res = coarse_gp.model_eval(model_keys)
                    print("\n")
        ...
        if N < Nmax:
            ...
            region = ((0.001, 4),(0.001, 2),(0.001, 5),(0.001, 4))
