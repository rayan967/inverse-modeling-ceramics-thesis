...
def adapt(gp, totalbudget, incrementalbudget, parameterranges,
    ...
    while currentcost < totalbudget:
        ...
        XC = np.array([])
        while XC.size == 0:
            XC = acquisitionfunction(gp,dfGLEE,normvar,w,XGLEE,epsphys,TOLAcqui)
            TOLAcqui*=0.99999
            if TOLAcqui < 0.1:
        ...
        if dim == 2:
            plotiteration(gp,w,normvar,N,Ngrad,XGLEE,XC,mcglobalerrorbefore,parameterranges, figurepath,counter)
        ...
        if sol.success == True:
            ...
            if indicesofchangedpoints[1].size == 0:
                ...
                for jj in range(indicesofchangedpoints[1].shape[0]):
                    ...
                    parameter = {"--h":gp.getX[currentFEMindex, :][0],
                                 "--x1":gp.getX[currentFEMindex, :][1],
                                 "--atol":1E-8,
                                 "--order":2}

                    #execpath = '/data/numerik/people/bzfsemle/simlopt/data/trainingdataFEM_stokes_2/'
