...
def estiamteweightfactors(var, X, dy, epsphys):
    ...
    if isinstance(epsphys, (np.floating, float)):
        SigmaLL = np.sqrt(epsphys)*np.eye(m)
...
def plotiteration(gp,w,var,N,Ngrad,XGLEE,XC,XCdummy,mcglobalerrorbefore,figurepath,counter):
    ...
    cntrvar = axvar.tricontourf(XGLEE[:,0], XGLEE[:,1], var[:,0], 15, cmap="RdBu_r")
    axvar.tricontour(XGLEE[:,0], XGLEE[:,1], var[:,0], 15, linewidths=0.5, colors='k')
    axvar.set(xlim=(-0.05, 3.05), ylim=(-0.05, 3.05))
    ...
    axw.set(xlim=(-0.05, 3.05), ylim=(-0.05, 3.05))
