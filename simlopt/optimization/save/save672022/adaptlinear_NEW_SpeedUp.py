...
def globalerrorestimate(epsilon, X, yt, hyperparameter, KXX, C1, C2, N,  SigmaLL, SigmaP, parameterranges):
    ...
    epscon = epsilon**2
    KXX = KXX+np.diagflat(epscon)
    invKXX = np.linalg.inv(KXX)

    for i, x in enumerate(X):
        KxX = (KXX-np.diagflat(epscon))[i, :].reshape((1, -1))
        alpha = np.linalg.solve(KXX, yt)
        df = dGPR(x.reshape((1, -1)), X, KxX, L)@alpha
        var = sigma**2 - KxX@invKXX@KxX.T
    ...
    #print("Used time sub: {}".format(t1-t0))
    return globalerrorestimate[0]  # 14.4

def gradientofglobalerrorestimate(epsilon, X, yt, hyperparameter,KXX, C1, C2,N, SigmaLL, SigmaP, parameterranges):
    ...
    for i,x in enumerate(X):
        ...
        KxX = (KXX-np.diagflat(epscon))[i,:].reshape((1,-1))
        alpha = np.linalg.solve(KXX, yt)
        df = (dGPR(x, X,KxX, Lhyper)@alpha).T
        var = sigma**2 - KxX@invKXX@KxX.T

        L = np.min(np.linalg.eig(np.outer(df, invsigma*df) + np.linalg.inv(SigmaP)+1E-4*np.eye(dim))[0])  # float
        ...
        for kk in range(N):
            ' Derivative dE / deps'
            dEdepsi = np.zeros((N, N))
            dEdepsi[kk, kk] = 2*(epsilon[0, kk])
            ' Derivative dsigma/deps'
            dvardeps[kk, 0] = KxX @ invKXX @ dEdepsi @ invKXX @ KxX.T

        errorsum += pwee * KR * dvardeps
...
def adapt(gp, N, NC, totalbudget, budgettospend,
    ...
    while currentcost < totalbudget:
        ...
        X,yt,Nall = gp.getX, gp.gety,gp.getdata[0] #All data inluding candiate points
        hyperparameter = gp.gethyperparameter
        KXX = kernelmatrix(X,X,hyperparameter)
        
        globarerror = globalerrorestimate(epsilon, X, yt, hyperparameter, KXX, C1, C2, Nall,
                                          SigmaLL, SigmaP, parameterranges)
        ...
        sol = scipy.optimize.minimize(globalerrorestimate, epsilon,
                                      args=(X, yt, hyperparameter,KXX, C1, C2, Nall,SigmaLL, SigmaP, parameterranges),
                                      method='SLSQP',
...
# =============================================================================
            ...
            newerrorestimate = globalerrorestimate(currentsol, X, yt, hyperparameter, KXX, C1, C2, Nall, 
                                              SigmaLL, SigmaP, parameterranges)[0]
