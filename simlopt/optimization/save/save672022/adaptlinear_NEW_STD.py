...
def maxpointwiseerror(C1, C2, df, SigmaLL, SigmaP, std, norm):
    ...
    for i in range(N):
        ...
        pointwiseerror = (12*C1*invnorm/L + 1/C2) * std[i, :]
...
def pointwiseerror(C1, C2, df, SigmaLL, SigmaP, std , norm):
    ...
    pointwiseerror = (12*C1*invnorm/L + 1/C2) *  std
...
def globalerrorestimate(epsilon, gp, C1, C2, NC, SigmaLL, SigmaP, ranges):
    ...
    for i in range(N):
        pwee = pointwiseerror(
            C1, C2, df[i, :], SigmaLL, SigmaP, std[i, :], np.inf)
        errorsum += pwee**2
...
def gradientofglobalerrorestimate(epsilon, gp, C1, C2, NC, SigmaLL, SigmaP, ranges):
    ...
    for i in range(N):
        ...
        dstdeps = np.zeros((N, 1))
        KxX = KXX[i, :].reshape((1, -1))
       
        for kk in range(N):
            dEdepsi = np.zeros((matsize, matsize))
            dEdepsi[kk, kk] = 2*(epsilon[0, kk])
            dstdeps[kk, 0] = KxX @ invK @ dEdepsi @ invK @ KxX.T
        
        errorsum +=  KR * KR *  dstdeps
        
    gradient = volofparameterspace/N * errorsum
    return gradient
...
def adapt(gp, N, NC, totalbudget, budgettospend,
          ...
          beta=1.4, sqpiter=1000, sqptol=1E-3):
    """
...
# =============================================================================
            ...
            if np.linalg.norm(newerrorestimate-globarerror) < TOLe:
                ...
                if currentcost < totalbudget:
                    ...
                    std = np.sqrt(gp.predictvariance(gp.getX))
                    maxpwe = maxpointwiseerror(
                        C1, C2, df, SigmaLL, SigmaP, std, np.inf)
