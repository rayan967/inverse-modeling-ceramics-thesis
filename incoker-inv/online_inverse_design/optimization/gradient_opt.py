...
def optimise_for_value(prop, X, property_name):
    ...
    for initial_point in initial_points:
        ...
        res = minimize(
            fun=lambda x: objective_function(x, prop, pipe, property_name, callback),
            jac=lambda x: objective_gradient(x, prop, pipe, property_name),
            x0=initial_point,
            bounds=bounds,
            method="L-BFGS-B",
        )
...
models = joblib.load("models/CTC_s_adapt.joblib")["models"]
...
optimise_for_value(15.1, X, property_name)
#########
# or random choice
# initial_points = X[np.random.choice(X.shape[0], NUM_STARTS, replace=False)]
...
for prop in prop_values:
    ...
    for initial_point in initial_points:
        ...
        """
        res = minimize(
            fun=lambda x: objective_function(x, prop, pipe, property_name),
            jac=lambda x: objective_gradient(x, prop, pipe, property_name),
            x0=initial_point,
            bounds=bounds,
            method="L-BFGS-B"
