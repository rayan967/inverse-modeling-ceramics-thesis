...
def main(config_path):
    ...
    parameterranges = np.array([
        config["parameterranges"]["VolumeFractionZirconia"],
        config["parameterranges"]["ChordLengthRatio"]
    ])
    ...
    if compute:
        GP_adapted = adapt_inc(gp, parameterranges, TOL, TOLAcqui, TOLrelchange, epsphys, X_test, y_test, execpath, runpath, output_stream, property_name, simulation_options)

    # Pass iteration number for failed run
    else:
        GP_adapted = adapt_inc(gp, parameterranges, TOL, TOLAcqui, TOLrelchange, epsphys, X_test, y_test, execpath, runpath, output_stream, property_name, simulation_options, iter_count)
