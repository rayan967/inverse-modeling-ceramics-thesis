...
def convert_x_to_microstructure(x, features):
    if len(features) == 3:
        volume_fraction_zro2 = x[0]
        chord_length_mean_al2o3 = x[1]
        chord_length_mean_ratio = x[2]

        # Construct the microstructure representation
        microstructure = {
            'volume_fraction_zro2': volume_fraction_zro2,
            'chord_length_mean_al2o3': chord_length_mean_al2o3,
            'chord_length_mean_ratio': chord_length_mean_ratio,
        }
    elif len(features) == 2:
        volume_fraction_zro2 = x[0]
        ...
        microstructure = {
            'volume_fraction_zro2': volume_fraction_zro2,
            'chord_length_mean_ratio': chord_length_ratio,
        }
...
def objective_function(x, desired_property, models, property_name, callback=None):
    ...
    return (discrepancy ** 2) + (uncertainty[0]**2  * 0.1)
...
models = joblib.load("../new_TC_model.joblib")["models"]
...
for prop in prop_values:
    ...
    optimal_volume_fractions_4.append(optimal_microstructure['volume_fraction_zro2'])
    optimal_rho.append(optimal_microstructure['chord_length_mean_ratio'])
