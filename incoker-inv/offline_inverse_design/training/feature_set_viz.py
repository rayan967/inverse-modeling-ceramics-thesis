...
kernels_with_white = [RBF() + WhiteKernel(), Matern() + WhiteKernel(), RBF() * DotProduct() + WhiteKernel(),
                      RationalQuadratic() + WhiteKernel(), DotProduct + WhiteKernel()]
feature_extraction_functions = {'8-Feature set': extract_XY, '3-Feature set': extract_XY_3,
...
for i, property_name in enumerate(considered_properties):
    ...
    for feature_set, extract_func in zip(['8-Feature set', '3-Feature set', '2-Feature set'],
                                         [extract_XY, extract_XY_3, extract_XY_2]):
